...
function startConstructionGraphicOne (loginId, containerMain, containerSection, survey, factor, component, generateGraphOne) {
    if ( survey === "SurveyOne") {
                ...
                if (this.readyState == 4 &&  this.status == 200) {
                    ...
                    for ( var i = 0 ; i < Object.keys(response.result).length ; i++ ) {
                        if (response.result[i].studentId === loginId) {
                            for ( var j = 0 ; j < Object.keys(response.result[i].groupData).length ; j++ ) {
                            ...
                            generateGraphOne (loginId, levelResponse, containerMain, containerSection, survey, factor, component, improve, enrich, persist, generateBtnSectionTwo);
                            console.log ("Improve: "+ improve); console.log ("Enrich: "+ enrich);   console.log ("Persist: "+ persist);      
                        }   
                    }
                }
            }
            ...
            if (this.readyState == 4 &&  this.status == 200) {
                ...
                if ( factor === "Methodical"){
                    ...
                    for ( var i = 0 ; i < Object.keys(response.result).length ; i++ ) {
                        if (response.result[i].studentId === loginId) {
                            for ( var j = 0 ; j < Object.keys(response.result[i].groupData).length ; j++ ) {
                                if (response.result[i].groupData[j].factorId === factor && response.result[i].groupData[j].componentId === component) {
                                    if (response.result[i].groupData[j].levelId === "VeryLow") {
                                        veryLow = response.result[i].groupData[j].cohortCount;
                                    }
                                    if (response.result[i].groupData[j].levelId === "Low") {
                ...
                generateGraphOne (loginId, levelResponse, containerMain, containerSection, survey, factor, component, veryLow, low, medium, high, veryHigh, generateBtnSectionTwo);
            }
        }
        surveyhttp.open ("GET", "data/surveyResult.json",true);
        surveyhttp.send();
...
function generateGraphOneSurveyOne (loginId, levelResponse, containerMain, containerSection, survey, factor, component, improve, enrich, persist, generateBtnSectionTwo){
    ...
    var groupImprove = d3.select("."+nameClass)
        .append("svg")
    ...
    var groupEnrich = d3.select("."+nameClass)
        .append("svg")
        .attr("width", maxWidth) 
    ...
    var groupPersist = d3.select("."+nameClass)
        .append("svg")
        .attr("width", maxWidth)
        .attr("height", maxHeight)
        .attr("id", "highLevel")
    ...
    var labelIndicatorsLow = d3.select(".levelIndicators"+factor)
        .append("text")
        .text("Nivel Bajo "+"("+improve+")")
        .attr("class", "indicatorLevel")
        .attr("id","indicatorLowLevel");

    var labelIndicatorsNormal = d3.select(".levelIndicators"+factor)
        .append("text")
        .text("Nivel Medio "+"("+enrich+")")
        .attr("class", "indicatorLevel")
        .attr("id","indicatorNormalLevel");
    
    var labelIndicatorsHigh = d3.select(".levelIndicators"+factor)
        .append("text")
        .text("Nivel Alto "+"("+persist+")")
        .attr("class", "indicatorLevel")
        .attr("id","indicatorHighLevel");

    generateBtnSectionTwo (containerMain, survey, factor, component, containerSectionTwo);
    //generateBtnSectionThree(containerMain, survey, factor, component,  containerSectionThree);
}

function generateGraphOneSurveyTwo (loginId, levelResponse, containerMain, containerSection, survey, factor, component, veryLow, low, medium, high, veryHigh, generateBtnSectionTwo) {
    
    var totalRow = Math.round(Math.max(veryLow,low,medium,high,veryHigh)/10 + 0.4); 
    ...
    var groupVeryLow = d3.select("."+nameClass) // Contenedor para los estudiantes en el nivel más bajo //
        .append("svg")
        .attr("width", maxWidth) 
        .attr("height", maxHeight)
        .attr("id", "veryLow") 
    ...
    var groupLow = d3.select("."+nameClass) // segundo contenedor para los estudiantes en el nivel bajo //
        .append("svg")
        .attr("width", maxWidth) 
        .attr("height", maxHeight) 
        .attr("id", "low") 
    ...
    var groupMedium = d3.select("."+nameClass) // Tercer contenedor para los estudiantes en el nivel medio //
        .append("svg")
        .attr("width", maxWidth) //ancho (lados)
        .attr("height", maxHeight) //largo (abajo)
        .attr("id", "medium")
    ...
    var groupHigh = d3.select("."+nameClass) // Cuarto contenedor para los estudiantes en el nivel medio //
        .append("svg")
        .attr("width", maxWidth) //ancho (lados)
        .attr("height", maxHeight) //largo (abajo)
        .attr("id", "high")
    ...
    var groupVeryHigh = d3.select("."+nameClass) 
        .append("svg")
        .attr("width", maxWidth)
        .attr("height", maxHeight) 
        .attr("id", "veryHigh")
...
function startConstructionGraphicTwo (loginId, containerSection, containerMain, survey, factor, component, generateGraphTwo) {
    if ( survey === "SurveyOne") {
        var surveyhttp = new XMLHttpRequest();
            surveyhttp.onreadystatechange = function() {
                if (this.readyState == 4 &&  this.status == 200) {
                    ...
                    generateGraphTwo (loginId, containerSection, containerMain, survey, factor, component,improve, enrich, persist, generateBtnSectionThree);
                }
            }
            surveyhttp.open ("GET", "data/surveyResult.json",true);
            surveyhttp.send();
        ...
        var surveyhttp = new XMLHttpRequest();
            surveyhttp.onreadystatechange = function() {
                if (this.readyState == 4 &&  this.status == 200) {
                    ...
                    generateGraphTwo(loginId, containerSection, containerMain, survey, factor, component, veryLow, low, medium, high, veryHigh, generateBtnSectionThree);
                }
            }
            surveyhttp.open ("GET", "data/surveyResult.json",true);
            surveyhttp.send();
...
function generateGraphTwoSurveyOne (loginId, containerSection, containerMain, survey, factor, component,improve, enrich, persist, generateBtnSectionThree) {
    
    var heightMax = 265, widthMax = 200;
    ...
    var groupImprove = d3.select("."+nameClass) // contenedor individual por categoria //
        .append("svg")
        .attr("width", widthMax) 
    ...
    if (Math.round(improve[0])+ Math.round(improve[1])+ Math.round(improve[2]) > 100) {
        improve[0]--; 
    }
    if (Math.round(improve[0])+ Math.round(improve[1])+ Math.round(improve[2]) === 99) {
        improve[2]++; 
    } 
    console.log(Math.round(improve[0])+ Math.round(improve[1])+ Math.round(improve[2]) );
   
    groupImprove.append("text")
        .text("Nivel Bajo " + "("+Math.round(improve[2])+" - "+Math.round(improve[1])+ " - " +Math.round(improve[0])+")")
        .attr("text-anchor","middle")
        .attr("x",100)
        .attr("y",260)
        .attr("fill","black")
    ...
    if (Math.round(enrich[0])+ Math.round(enrich[1])+ Math.round(enrich[2]) > 100) {
        enrich[0]--; 
    }
    if (Math.round(enrich[0])+ Math.round(enrich[1])+ Math.round(enrich[2]) === 99) {
        enrich[2]++; 
    } 
    //console.log(Math.round(enrich[0])+ Math.round(enrich[1])+ Math.round(enrich[2]) )
    groupEnrich.append("text")
        .text("Nivel Medio " + "("+Math.round(enrich[2])+" - "+Math.round(enrich[1])+ " - " +Math.round(enrich[0])+")")
        .attr("text-anchor","middle")
        .attr("x",100)
        .attr("y",260)
        .attr("fill","black")
    ...
    if (Math.round(persist[0])+ Math.round(persist[1])+ Math.round(persist[2]) > 100) {
        persist[0]--; 
    }
    if (Math.round(persist[0])+ Math.round(persist[1])+ Math.round(persist[2]) === 99) {
        persist[2]++; 
    } 
    //console.log(Math.round(persist[0])+ Math.round(persist[1])+ Math.round(persist[2])  );

    groupPersist.append("text")
    .text("Nivel Alto " + "("+Math.round(persist[2])+" - "+Math.round(persist[1])+ " - " +Math.round(persist[0])+")")
    .attr("text-anchor","middle")
    .attr("x",100)
    .attr("y",260)
    .attr("fill","black")
    ...
    //  Nivel Bajo
    var width = 18, height = 18, posX = 0, posY = 40, aux = posX, jumpLine = 1;
    for (var i = 1; i <= Math.round(improve[2]); i++) {
        if (i%10 === 0) {         
            groupImprove.append("rect")
                .attr("x",aux)
                .attr("y",posY)
                .attr("width",width)
                .attr("height",height)
    ...
    document.getElementById(containerSection).appendChild(newContainerTwo);
    newContainerTwo.innerHTML = "<br/> <center><svg width='18' height = '18' > <rect width='18' height='18' style='fill:#a1d99b'/></svg> Aprobaron todas sus asignaturas &nbsp;&nbsp;&nbsp; <svg width='18' height = '18' > <rect width='18' height='18' style='fill:#fed976'/></svg> Reprobaron una asignatura &nbsp;&nbsp;&nbsp; <svg width='18' height = '18' > <rect width='18' height='18' style='fill:#ffab91'/></svg>   Reprobaron dos o más asignaturas </center>";

    generateBtnSectionThree(containerMain, survey, factor, component,  containerSectionThree);
}

function generateGraphTwoSurveyTwo (loginId, containerSection, containerMain, survey, factor, component, veryLow, low, medium, high, veryHigh, generateBtnSectionThree) {
    var heightMax = 200;
    var widthMax = 160;
    ...
    var groupVeryLow = d3.select("."+nameClass) // Sección nivel másbajo //
        .append("svg")
        .attr("width", widthMax) 
        .attr("height", heightMax) 
        .attr("id", "veryLowTwo") 
    ...
    groupVeryLow.append("text") // Contenido con los porcentajes //
        .attr("x",80)
        .attr("y",30)
        .attr("text-anchor","middle")
    ...
    var width = 14, height = 14, posX = 0, posY = 40, aux = posX, jumpLine = 1;
    for (var i = 1; i <= good; i++) {
        if (i%10 === 0) {         
            groupVeryLow.append("rect")
                .attr("x",aux)
                .attr("y",posY)
                .attr("width",width) 
                .attr("height",height)
    ...
    var groupLow = d3.select("."+nameClass)
        .append("svg")
        .attr("width", widthMax) 
        .attr("height", heightMax) 
        .attr("id", "lowTwo") 
    ...
    groupLow.append("text") // Contenido con los porcentajes //
        .attr("x",80)
        .attr("y",30)
        .attr("text-anchor","middle")
        .text("("+good+" - "+normal+" - "+sad+")")
        ...
        posY = 40; aux = posX, jumpLine = 1;
        for (var i = 1; i <=  good; i++) {
            if (i%10 === 0) {         
                groupLow.append("rect")
                    .attr("x",aux)
                    .attr("y",posY)
                    .attr("width",width) 
                    .attr("height",height)
    ...
    var groupMedium = d3.select("."+nameClass)
        .append("svg")
        .attr("width", widthMax) 
        .attr("height", heightMax)
        .attr("id", "mediumTwo") 
    ...
    groupMedium.append("text") // Contenido con los porcentajes //
        .attr("x",80)
        .attr("y",30)
        .attr("text-anchor","middle")
        .text("("+good+" - "+normal+" - "+sad+")")
    ...
    jumpLine = 1; posY = 40; aux = posX;
    for (var i = 1; i <= good; i++) {
        if (i%10 === 0) {               
            groupMedium.append("rect")
                .attr("x",aux)
                .attr("y",posY)
                .attr("width",width) 
                .attr("height",height)
    ...
    var groupHigh = d3.select("."+nameClass)
        .append("svg")
        .attr("width", widthMax) 
        .attr("height", heightMax) 
        .attr("id", "highTwo") 
    ...
    groupHigh.append("text") // Contenido con los porcentajes //
        .attr("x",80)
        .attr("y",30)
        .attr("text-anchor","middle")
        .text("("+good+" - "+normal+" - "+sad+")")
    ...
    groupVeryHigh.append("text") // Contenido con los porcentajes //
        .attr("x",80)
        .attr("y",30)
        .attr("text-anchor","middle")
        .text("("+good+" - "+normal+" - "+sad+")")
    ...
    newContainerTwo.setAttribute("class", "symbologyGraph");
    document.getElementById(containerSection).appendChild(newContainerTwo);
    newContainerTwo.innerHTML = " <br/><center> <svg width='14' height = '14' > <rect width='14' height='14' style='fill:#a1d99b'/> </svg> Aprobaron todas sus asignaturas. &nbsp;&nbsp;&nbsp; <svg width='14' height = '14' > <rect width='14' height='14' style='fill:#fed976'/></svg> Reprobaron una asignatura. &nbsp;&nbsp;&nbsp; <svg width='14' height = '14' > <rect width='14' height='14' style='fill:#ffab91'/></svg>   Reprobaron dos o más asignaturas.</center>";

    generateBtnSectionThree(containerMain, survey, factor, component,  containerSectionThree);
}

function markPosition (category, height, widthx) { // Añade marcador de posicion en función de los puntajes obtenidos //
